...
url5= "http://api.dev.wgcloud.net/v1/packages"
headers = {'authorization': "Bearer " + accesstoken}
...
#Return a list with only package entries
packlist = []
counter = 0 
for entry in fqnlist:
    if "package" in entry:
        packlist.append(str(entry))
        counter = counter + 1
print("Packages: " + str(packlist))
print("There are " + str(counter) + " packages") 
